...
class KQQuestions extends Component {
  ...
  async fetchdata() {
    const response = await http.get(`/allQuestions`);
    const { data } = response;
  ...
  handleenter = () => {
    ...
    if (s1.name === "") {
      ...
      for (;;) {
        ...
        const len = s1.questions.length;
        const index = Math.floor(Math.random() * len);
        const fnd = s1.data.find((a) => a === s1.questions[index]);
  ...
  render() {
    const { questions, show, name, err, qindex, errcount, text, data, error } = this.state;
    console.log(error);
    console.log(show);
    if (show === 0) {
      return (
        <div className="container text-center p-4">
          ...
          <button
            ...
            onClick={() => this.handleenter(0, questions.length - 1)}
          >
            Enter
          </button>
        </div>
      ...
      return (
        <div className="container text-center p-4">
          ...
          <button
            ...
            onClick={() => this.handlebtn(data[qindex].opt1)}
          >
            {data[qindex].opt1}
            <br />
          </button>
          <button
            className="btn btn-primary mx-4"
            onClick={() => this.handlebtn(data[qindex].opt2)}
          >
            {data[qindex].opt2}
            <br />
          </button>
          <button
            className="btn btn-primary mx-4"
            onClick={() => this.handlebtn(data[qindex].opt3)}
          >
            {data[qindex].opt3}
            <br />
          </button>
      ...
      return (
        <div className="container text-center p-4">
          ...
          <h5>
            Error Count : <span className="text-danger">{errcount}</span>
          </h5>
